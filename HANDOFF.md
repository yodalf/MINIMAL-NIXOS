...
- The old 2017 instance `totos` (unlabeled, 45.77.78.141, id `b3506f78-9680-423f-85c3-a4a24b21d147`, 20 GB disk, Arch Linux) is stopped and still billed $3.50/month. Its disk was inventoried and backed up on 2026-09-03 by booting it from the v0.2 installer ISO (ddclient never runs that way; mount `/dev/vda1` read-only): `~/Backups/totos/totos-2026-09-03.tgz` on the Mac (1.9 GB, 78060 entries, sha256 starts `5c525299`; a copy sits in `~/.cache/minimal-nixos/totos/` on the dev VM). It holds `/home/realo` (minus google-cloud-sdk, `.cache`, `.npm`), `/root`, `/srv` (the pre-Hugo static sites: `http`, `http.JKL`, `http.SHOW`, `http_2017-05-14`), `/srv.ORI`, `/etc` (ddclient.conf with the Dyn login, httpd config, `letsencrypt/`, `systemd/system/realo.timer`), `/opt`. Notable inside `/home/realo`: `WORK/WWW/Hugo` (1.8 GB, older Hugo trees), `WORK/KRAKEN`, `WORK/VPN`, `WORK/vmkit`, `WORK/compute-archlinux-image-builder` (git), `WG/` + `wg0.conf` (a WireGuard server on UDP 1194 with 9 peers, private keys included; same in `/root/WG`), `.ssh/google_compute_engine` key, `.config/gcloud` credentials, `hello_owl` (a WebAssembly demo). Not backed up: `/var` (2 GB logs, 1.5 GB pacman cache, 760 MB docker images), `/usr`. Boot it from the ISO again if anything else is wanted; to shut it down use `POST /instances/<id>/halt` right after `iso/detach`, since the detach restarts it and a normal boot brings ddclient back. Destroy it once the backup is judged sufficient.
